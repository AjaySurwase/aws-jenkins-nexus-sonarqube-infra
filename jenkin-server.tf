...
resource "aws_instance" "jenkins-server" {               # we are creating a new instance for jenkins-server
  ...
  user_data            = file("./jenkins.sh")            # this is the script that will be executed during the creation of the instance
  ...
  tags = {
    Name = "PNC-jenkins-server" # this will provide name to instance 
  }
}

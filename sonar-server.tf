...
resource "aws_instance" "sonar-server" {          # we are creating a new instance for jenkins-server
  ...
  user_data     = file("./sonar.sh")              # this is the script that will be executed during the creation of the instance
  ...
  tags = {
    Name = "PNC-sonar-server" # this will provide name to instance 
  }
}
